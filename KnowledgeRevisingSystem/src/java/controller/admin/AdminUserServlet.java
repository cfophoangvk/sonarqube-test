    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (action.equals("Update")) {
            ...
            if (submit == null) {
                ...
                boolean flag = true;//Create a flag to mark whether there is an error or not
                int err=1;
                Vector<User> vUser = daoUser.getAll("select * from user where user_id !="+userId+" and user_name='"+userName+"'");
                if(!vUser.isEmpty()){
                    flag = false;
                    err*=2;
                }
                vUser = daoUser.getAll("select * from user where user_id !="+userId+" and email='"+email+"'");
                if(!vUser.isEmpty()){
                    flag = false;
                    err*=3;
                }
                vUser = daoUser.getAll("select * from user where user_id !="+userId+" and phone='"+phone+"'");
                if(!vUser.isEmpty()){
                    flag = false;
                    err*=5;
                }               
                //In case flag = false, it will return to the website for the user to re-do
                if (flag == false) {
                    response.sendRedirect("/krs/AdminUserServlet?action=Update&userId=" + userId+"&err="+err);
                } //In case flag = true, it will return to the user list page
                else {
